...
class SchedulingController:
    ...
    def set_maintenance_mode(self) -> None:
        ...
        if state == States.Maintenance or state == States.StoppingDueToMaintenance:
            error_message = f"Conflict - Call to set maintenance mode was given while in state {state}."
            self.logger.warning(error_message)
            raise HTTPException(
                status_code=HTTPStatus.CONFLICT,
                detail=error_message,
    ...
    def release_maintenance_mode(self) -> None:
        ...
        if state != States.Maintenance:
            error_message = f"Conflict - Release maintenance mode command received in invalid state - State: {state}"
            self.logger.warning(error_message)
            raise HTTPException(
                status_code=HTTPStatus.CONFLICT,
                detail=error_message,
