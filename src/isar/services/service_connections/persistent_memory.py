...
def read_persistent_robot_state_is_maintenance_mode(connection_string: str, robot_id: str):
    ...
    with Session(engine) as session:
        ...
        if read_persistent_state is None:
            raise NoSuchRobotException(f"No robot in persistent storage with id {robot_id}")

        return read_persistent_state.is_maintenance_mode

def change_persistent_robot_state_is_maintenance_mode(connection_string: str, robot_id: str, value: bool):
    engine = sqlalchemy.create_engine(connection_string)
