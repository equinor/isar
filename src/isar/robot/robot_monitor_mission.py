...
from robot_interface.telemetry.payloads import TaskPayload
...
class RobotMonitorMissionThread(Thread):
    ...
    def _get_task_status(self, task_id) -> TaskStatus:
        ...
        raise RobotTaskStatusException(error_description=failed_task_error.error_description)
    ...
    def _get_mission_status(self, mission_id) -> MissionStatus:
        ...
        raise RobotMissionStatusException(error_description=failed_mission_error.error_description)
    ...
    def run(self) -> None:
        ...
        while not self.signal_thread_quitting.wait(0):
            ...
            if current_task:
                ...
                if is_finished(new_task_status):
                    ...
                    if should_upload_inspections(current_task):
                        self.robot_service_events.request_inspection_upload.trigger_event((current_task, self.current_mission))
            ...
            if new_mission_status != last_mission_status:
                ...
                self.robot_service_events.mission_status_updated.trigger_event(new_mission_status)

            if new_mission_status not in [MissionStatus.NotStarted, MissionStatus.InProgress] and current_task is None:
                break

        self.logger.info("All tasks in current mission have completed")
