    ...
    def _run(self) -> None:
        while True:
            if not self.robot_status_thread:
                self.robot_status_thread = ThreadedRequest(request_func=self.state_machine.robot.robot_status)
                self.robot_status_thread.start_thread(name="State Machine Offline Get Robot Status")

            try:
                robot_status: RobotStatus = (
                    self.robot_status_thread.get_output()
                )
