...
class AwaitNextMission(State):
    ...
    def _check_and_handle_start_mission_event(
        ...
        if start_mission:
            ...
            if self._is_return_home_mission(mission=start_mission):
                self.state_machine.request_return_home_from_api()  # type: ignore
                return True
            else:
    ...
    def _is_return_home_mission(self, mission: StartMissionMessage) -> bool:
        if len(mission.mission.tasks) != 1:
            return False
        if mission.mission.tasks[0].type != TaskTypes.ReturnToHome:
            return False
        return True
