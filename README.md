...
The tasks of a mission are selected based on a task selector module, defined by the `TASK_SELECTOR` configuration variable. The default selector is `sequential`.
...
Optionally, the `ìnitialize()` function may be extended by supplementing the parameter list or function body:
