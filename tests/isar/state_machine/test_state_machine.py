...
from tests.mocks.robot_interface import (
    MockRobot,
    MockRobotDockedToIdleTest,
    MockRobotOfflineToDockedTest,
    MockRobotOfflineToIdleTest,
    MockRobotBlockedProtectiveStopToIdleTest,
)
...
def test_state_machine_offline_to_idle(
    ...
    robot_service_thread.robot_service.robot = MockRobotOfflineToIdleTest(
    ...
    assert state_machine_thread.state_machine.transitions_list == deque(
        [States.UnknownStatus, States.Offline, States.Idle]
    )


def test_state_machine_blocked_protective_stop_to_idle(
    mocker, state_machine_thread, robot_service_thread
) -> None:
    # Robot status check happens every 5 seconds by default, so we mock the behavior
    # to poll for status imediately
    ...
    robot_service_thread.robot_service.robot = MockRobotBlockedProtectiveStopToIdleTest(
        robot_service_thread.robot_service.shared_state.state
    )
    ...
    assert state_machine_thread.state_machine.transitions_list == deque(
        [States.UnknownStatus, States.BlockedProtectiveStop, States.Idle]
    )


def test_state_machine_docked_to_idle(
    mocker, state_machine_thread, robot_service_thread
) -> None:
    # Robot status check happens every 5 seconds by default, so we mock the behavior
    # to poll for status imediately
    ...
    robot_service_thread.robot_service.robot = MockRobotDockedToIdleTest(
        robot_service_thread.robot_service.shared_state.state
    )
    ...
    assert state_machine_thread.state_machine.transitions_list == deque(
        [States.UnknownStatus, States.Docked, States.Idle]
    )


def test_state_machine_offline_to_docked(
    mocker, state_machine_thread, robot_service_thread
) -> None:
    # Robot status check happens every 5 seconds by default, so we mock the behavior
    # to poll for status imediately
    ...
    robot_service_thread.robot_service.robot = MockRobotOfflineToDockedTest(
        robot_service_thread.robot_service.shared_state.state
    )
    ...
    assert state_machine_thread.state_machine.transitions_list == deque(
        [States.UnknownStatus, States.Offline, States.Docked]
    )
